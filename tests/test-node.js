const LibRubyParser = require("../build/nodejs");
...
function test_ast() {
  ...
  const expected = new LibRubyParser.nodes.Send(
    // recv
    new LibRubyParser.nodes.Int("2", undefined, new LibRubyParser.Loc(0, 1)),
    // method_name
    "+",
    // args,
    [
...
function test_tokens() {
  ...
  const expected = [
    new LibRubyParser.Token(
      307,
      new TextEncoder("utf-8").encode("foo"),
      new LibRubyParser.Loc(0, 3)
    ),
    new LibRubyParser.Token(
      314,
      new TextEncoder("utf-8").encode("42"),
...
function test_comments() {
  ...
  const expected = [
    new LibRubyParser.Comment(new LibRubyParser.Loc(0, 6), "inline"),
    new LibRubyParser.Comment(new LibRubyParser.Loc(6, 21), "document"),
  ];
  assert.deepEqual(actual, expected);
}
...
function test_decoder_ok() {
  ...
  const output = LibRubyParser.parse(input, null, (encoding, input) => {
    decoder_called = true;
  ...
  assert.deepEqual(
    output.tokens[0].token_value,
    new TextEncoder("utf-8").encode("3")
  );
  assert.deepEqual(
...
function test_decoder_err() {
  ...
  assert.deepEqual(output.diagnostics, [
    new LibRubyParser.Diagnostic(
      "error",
      new LibRubyParser.messages.EncodingError(
        'DecodingError("JS[Unsupported encoding two-is-three]")'
      ),
